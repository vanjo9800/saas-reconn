...
func VirusTotalQuery(domain string, apikey string, verbosity int) (subdomains []string) {
	...
	if len(apikey) == 0 {
		log.Printf("[VirusTotal] No VirusTotal API key found")
		return subdomains
	}
	...
	for {
		...
		if err != nil {
			log.Printf("[VirusTotal] Could not connect to VirusTotal for %s: %s", domain, err)
			break
		}
		...
		if err != nil {
			log.Printf("[VirusTotal] Invalid JSON response for %s", domain)
			break
		}
